...
def get_by_id_should_retrieve_same_object(member_store, member2):
    member2_retrieved = member_store.get_by_id(member2._id)
...
def update_should_modify_object(member_store, member3):
    member3_copy = models.Member(member3._name, member3._age)
    member3_copy._id = 3
    ...
    print(member3_copy)
    member3_copy._name = "John"
    member_store.update(member3_copy)
    print(member_store.get_by_id(member3._id))
...
def create_posts(members_instances):
    ...
    post1 = models.Post("Agriculture", "Agriculture is amazing", members_instances[0]._id)
    post2 = models.Post("Engineering", "I love engineering", members_instances[0]._id)

    post3 = models.Post("Medicine", "Medicine is great", members_instances[1]._id)
    post4 = models.Post("Architecture", "Spectacular art", members_instances[1]._id)
    post5 = models.Post("Astronomy", "Space is awesome", members_instances[1]._id)

    post6 = models.Post("Geology", "Earth is our friend", members_instances[2]._id)
    post7 = models.Post("ComputerSci", "Our passion", members_instances[2]._id)
    post8 = models.Post("Algorithms", "Yeah, more of that", members_instances[2]._id)
    post9 = models.Post("Operating Systems", "Ewww", members_instances[2]._id)
